...
class LineArcBentUpView(ctx : Context) : View(ctx) {
    ...
    data class LABUNode(var i : Int = 0, val state : State = State()) {

        private var next : LABUNode? = null
        private var prev : LABUNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLABUNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LABUNode {
            var curr : LABUNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineArcBentUp(var i : Int) {

        private var curr : LABUNode = LABUNode(0)
    ...
    companion object {
        fun create(activity : Activity) : LineArcBentUpView {
            val view : LineArcBentUpView = LineArcBentUpView(activity)
