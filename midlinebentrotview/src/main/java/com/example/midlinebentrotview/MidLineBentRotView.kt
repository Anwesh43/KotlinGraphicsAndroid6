package com.example.midlinebentrotview
...
fun Canvas.drawMidLineBentRot(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawMLBRNode(i : Int, scale : Float, paint : Paint) {
...
class MidLineBentRotView(ctx : Context) : View(ctx) {
    ...
    data class MLBRNode(var i : Int = 0, val state : State = State()) {

        private var next : MLBRNode? = null
        private var prev : MLBRNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawMLBRNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : MLBRNode {
            var curr : MLBRNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
