package com.example.lineintoquarterarcview
...
fun Canvas.drawLineIntoQuarterArc(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2) * dsc(4), h / 2) {
        ...
        drawArc(RectF(-size / 2, -size / 2, size / 2, size / 2), 0f, sweep * dsc(3), true, paint)
    }
}
...
fun Canvas.drawLIQANode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineIntoQuarterArc(scale, w, h, paint)
}

class LineIntoQuarterArcView(ctx : Context) : View(ctx) {
    ...
    data class LineIntoQuarterArc(var i : Int = 0) {

        private var curr : LIQANode = LIQANode(0)
