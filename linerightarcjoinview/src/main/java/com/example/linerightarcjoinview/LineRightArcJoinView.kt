...
class LineRightArcJoinView(ctx : Context) : View(ctx) {
    ...
    data class LineRightArcJoin(var i : Int) {

        private var curr : LRAJNode = LRAJNode(0)
    ...
    data class Renderer(var view : LineRightArcJoinView) {

        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val lraj : LineRightArcJoin = LineRightArcJoin(0)

        fun render(canvas : Canvas) {
            lraj.draw(canvas, paint)
            animator.animate {
                lraj.update {
                    animator.stop()
                }
    ...
    companion object {
        fun create(activity : Activity) : LineRightArcJoinView {
            val view : LineRightArcJoinView = LineRightArcJoinView(activity)
