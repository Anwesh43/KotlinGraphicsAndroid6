package com.example.linearcsweepdownview
...
fun Canvas.drawLineArcSweepDown(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2 + (h / 2) * dsc(3)) {
        ...
        drawArc(RectF(-size, -size, size, size), -(deg - rot), deg * dsc(1), false, paint)
    }
}
...
fun Canvas.drawLASDNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineArcSweepDown(scale, w, h, paint)
}

class LineArcSweepDownView(ctx : Context) : View(ctx) {
    ...
    data class LineArcSweepDown(var i : Int) {
