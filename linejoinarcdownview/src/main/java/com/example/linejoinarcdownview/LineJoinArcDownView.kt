...
class LineJoinArcDownView(ctx : Context) : View(ctx) {
    ...
    data class LineJoinArcDown(var i : Int) {

        private var curr : LJADNode = LJADNode(0)
    ...
    data class Renderer(var view : LineJoinArcDownView) {

        private val ljad : LineJoinArcDown = LineJoinArcDown(0)
        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            ljad.draw(canvas, paint)
            animator.animate {
                ljad.update {
                    animator.stop()
                }
