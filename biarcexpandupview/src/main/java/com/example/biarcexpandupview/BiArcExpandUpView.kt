package com.example.biarcexpandupview
...
class BiArcExpandUpView(ctx : Context) : View(ctx) {
    ...
    data class BAEUNode(var i : Int = 0, val state : State = State()) {

        private var next : BAEUNode? = null
        private var prev : BAEUNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBAEUNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : ()-> Unit) : BAEUNode {
            var curr : BAEUNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BiArcExpandUp(var i : Int) {

        private var curr : BAEUNode = BAEUNode(0)
